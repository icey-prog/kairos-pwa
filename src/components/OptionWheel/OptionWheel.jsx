...
const OptionWheel = ({
  ...
  const handleItemClick = useCallback(
    index => {
      ...
      // Tap explicite (pas un drag) : laisse le parent agir (ex: naviguer).
  ...
  useEffect(() => {
    // Fix StrictMode (double-mount) : sans relance ici + reset de rafRef à
    // null, le cleanup du 1er mount annule le rAF et startLoop ne redémarre
    // jamais — les items restent empilés sans transform. (bug upstream)
    // Premier layout SYNCHRONE : si le rAF est throttlé (onglet en arrière-plan,
    // économie d'énergie), la roue s'affiche quand même correctement.
    lastRef.current = performance.now();
  ...
  return (
    ...
    >
      {items.map((item, index) => {
        // Item riche {label, icon, color} ou simple string — rétro-compatible.
        ...
        return (
          <div
            ...
            style={color ? { '--ow-active-color': color } : undefined}
            onClick={() => handleItemClick(index)}
          >
            {icon && <span className="option-wheel__icon">{icon}</span>}
            {label}
          </div>
        );
      })}
